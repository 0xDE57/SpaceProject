...
import com.badlogic.gdx.math.ShewchukExactPredicates;
 ...
 * @author Nathan Sweet
 * @author Berstanio */
public class DoubleDelaunayTriangulator {
    ...
    private final IntArray completedTriangles = new IntArray(false, 16);
    ...
    public IntArray computeTriangles(double[] points, int offset, int count, boolean sorted) {
        IntArray completedTriangles = this.completedTriangles;
        ...
        for (int pointIndex = offset; pointIndex < end; pointIndex += 2) {
            ...
            for (int triangleIndex = triangles.size - 1; triangleIndex >= 0; triangleIndex -= 3) {
                ...
                switch (circumCircle(points, end, x, y, p1, p2, p3)) {
                    case COMPLETE:
                        completedTriangles.add(p1, p2, p3);
            ...
            for (int i = 0, n = edges.size; i < n; i += 2) {
                ...
                triangles.add(p1, p2, pointIndex);
            }
            edges.clear();
        }
        ...
        int[] completedTrianglesArray = completedTriangles.items;
        ...
        if (!sorted) {
            ...
            for (int i = 0; i < n; i++)
                completedTrianglesArray[i] = (originalIndicesArray[completedTrianglesArray[i] / 2] * 2);
        }
        
        // Adjust triangles to start from zero and count by 1, not by vertex x,y coordinate pairs.
        if (offset == 0) {
            for (int i = 0; i < n; i++)
                completedTrianglesArray[i] = (completedTrianglesArray[i] / 2);
        } else {
            for (int i = 0; i < n; i++)
                completedTrianglesArray[i] = ((completedTrianglesArray[i] - offset) / 2);
        }
     ...
     * </ul>
     */
    private int circumCircle(double[] points, int end, double xp, double yp, int p1, int p2, int p3) {
    ...
    public void trim(IntArray triangles, double[] points, double[] hull, int offset, int count) {
        int[] trianglesArray = triangles.items;
        for (int i = triangles.size - 1; i >= 0; i -= 3) {
            int p1 = trianglesArray[i - 2] * 2;
            int p2 = trianglesArray[i - 1] * 2;
            int p3 = trianglesArray[i] * 2;
            triangleCentroid(points[p1], points[p1 + 1], points[p2], points[p2 + 1], points[p3], points[p3 + 1],
                centroid);
            if (!isPointInPolygon(hull, offset, count, centroid.x, centroid.y)) {
    ...
    static public Vector2 triangleCentroid(double x1, double y1, double x2, double y2, double x3, double y3, Vector2 centroid) {
        centroid.x = (float) ((x1 + x2 + x3) / 3);
        centroid.y = (float) ((y1 + y2 + y3) / 3);
        return centroid;
    }
    
    public static boolean isPointInPolygon(double[] polygon, int offset, int count, double x, double y) {
        boolean oddNodes = false;
        double sx = polygon[offset], sy = polygon[offset + 1], y1 = sy;
        int yi = offset + 3;
        for (int n = offset + count; yi < n; yi += 2) {
            double y2 = polygon[yi];
            if ((y2 < y && y1 >= y) || (y1 < y && y2 >= y)) {
                double x2 = polygon[yi - 1];
